...
class AIService {
  ...
  private createChatWorkflow(request: AIWorkflowRequest) {
    const systemPrompt =
      request.parameters?.systemPrompt || "You are a helpful AI assistant.";
        ...
        nodes: [
          {
            ...
            parameters: {
              system:
                "You are a creative writing assistant. Help generate engaging content.",
  ...
  private createAnalysisWorkflow(request: AIWorkflowRequest) {
    const analysisFocus =
      request.parameters?.analysisFocus || "insights and recommendations";
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        if (status.status === "completed") {
          ...
          throw new Error(
            `Workflow execution failed: ${status.error || "Unknown error"}`,
          );
        }
  ...
  async generateBlock(request: any): Promise<any> {
    throw new Error(
      "Block generation is deprecated. Use executeAIWorkflow instead.",
    );
  }

  async validateGeneratedCode(code: string): Promise<any> {
    throw new Error(
      "Code validation is deprecated. Use workflow-based AI execution instead.",
    );
  }

  async deployGeneratedBlock(block: any): Promise<void> {
    throw new Error(
      "Block deployment is deprecated. Use workflow-based AI execution instead.",
    );
  }
}
