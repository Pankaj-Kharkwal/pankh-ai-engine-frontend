...
import { Plus, Code, Database, Zap, Mail, Search, Filter, Loader2, AlertCircle, Power, PowerOff, Layers, Grid, BarChart3, Settings } from 'lucide-react'
import { useBlocks, useBlockCategories, useRegistryStats, useGenerateBlock, useEnableBlock, useDisableBlock } from '../hooks/useApi'
...
const getBlockIcon = (blockType: string) => {
    if (blockType.toLowerCase().includes('data') || blockType.toLowerCase().includes('input')) return Database
    if (blockType.toLowerCase().includes('ai') || blockType.toLowerCase().includes('llm')) return Zap
    if (blockType.toLowerCase().includes('email') || blockType.toLowerCase().includes('mail')) return Mail
    if (blockType.toLowerCase().includes('process') || blockType.toLowerCase().includes('filter')) return Settings
    if (blockType.toLowerCase().includes('log') || blockType.toLowerCase().includes('report')) return BarChart3
    return Code
}
...
export default function Blocks() {
    ...
    const filteredBlocks = Array.isArray(blocks) ? blocks.filter((block: any) =>
    ...
    return (
        <div className="min-h-screen bg-gray-50">
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            ...
                            {registryStats && (
                                <span className="ml-2 font-medium text-gray-700">
                                    (<span className="font-bold text-green-600">{(registryStats as any).enabled_blocks}</span> / {(registryStats as any).total_blocks} Active)
                                </span>
                            )}
                        </p>
                    </div>
                ...
                {registryStats && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100 transform hover:translate-y-[-2px] transition duration-300">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm font-medium text-gray-500 uppercase tracking-wider">Total Blocks</p>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100 transform hover:translate-y-[-2px] transition duration-300">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm font-medium text-gray-500 uppercase tracking-wider">Plugins</p>
                                    <p className="text-4xl font-extrabold text-gray-900 mt-2">
                    ...
                    {filteredBlocks.length > 0 ? (
                                        ...
                                        <div className="flex-grow">
                                            <div className="flex items-start justify-between mb-4">
                                                ...
                                                <div className="flex space-x-2 items-center">
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleToggleBlock(block); }}
                                                    ...
                                                    >
                                                        {block.enabled ? <PowerOff className="w-5 h-5" /> : <Power className="w-5 h-5" />}
                                                    </button>
                                                    
                                                    <div className={`px-3 py-1 text-xs font-bold rounded-full uppercase tracking-widest ${block.enabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                            ...
                                            <p className="text-sm text-gray-500 mb-6 line-clamp-3">
                                                {block.manifest?.summary || `Block type: ${block.type}. No summary provided in manifest.`}
                                            </p>
                                        </div>

                                        {/* Footer: Category Tag and Action Button */}
                                        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                                            <span className={`text-xs ${color} bg-gray-100 px-3 py-1 rounded-full font-bold uppercase`}>
                                                {category}
                                            </span>
                                            <button
                        ...
                        <div className="text-center py-20 px-4 bg-gray-50 border-4 border-dashed border-gray-200 rounded-2xl">
                            ...
                            <p className="text-gray-500 mb-8 max-w-md mx-auto">
                                We couldn't find any blocks matching your search or filter criteria.
                                Try adjusting your search term or category, or create a new block.
                            </p>
                            <button 
                                onClick={() => setShowGenerateModal(true)}
                ...
                >
                    ...
                    >
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                <label htmlFor="block-description" className="block text-base font-semibold text-gray-800 mb-3">
                                ...
                                />
                                <p className="text-xs text-gray-500 mt-2">The more detail you provide, the better the generated block will be.</p>
                            </div>
                            ...
                            <div className="flex justify-end space-x-3 pt-4">
                                ...
                                <button
                                    ...
                                    className={`px-6 py-3 text-base font-medium text-white rounded-xl shadow-lg transition-all duration-300
                                        ${!generateDescription.trim() || generateBlockMutation.isPending
            ...
            <AIAssistant
                context={selectedBlock ? `Currently viewing ${selectedBlock.type} block` : 'Block management and configuration'}
                contextType="block"
                suggestions={[
