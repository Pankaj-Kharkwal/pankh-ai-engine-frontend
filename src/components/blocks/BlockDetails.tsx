...
import { Play, CheckCircle, XCircle, Loader2, Info, Settings, FileJson, TestTube, ScrollText, BarChart3, Grid, Activity, Zap } from 'lucide-react';
...
const InfoContent: React.FC<{ block: any }> = ({ block }) => (
  <div className="space-y-6">
    ...
    <div className="flex items-center space-x-2">
      ...
      <div
        className={`w-3 h-3 rounded-full ${block?.enabled ? "bg-green-500" : "bg-red-500"}`}
      />
      ...
      <div className="space-y-6">
        ...
        {testResults.result && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 shadow-sm">
            <h5 className="font-semibold text-blue-900 mb-2 border-b border-blue-200 pb-1">Final Result</h5>
        ...
        {testResults.error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 shadow-sm">
            <h5 className="font-semibold text-red-900 mb-2 border-b border-red-200 pb-1">Error Details</h5>
        ...
        {(testResults.validation_info || (testResults.validation_errors && testResults.validation_errors.length > 0)) && (
          <div className="space-y-4">
            {testResults.validation_info && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 shadow-sm">
                <h5 className="font-semibold text-green-900 mb-2 border-b border-green-200 pb-1">Validation Info</h5>
                <div className="text-sm text-green-800 space-y-1">
                  <div>
                    <strong>Block Type:</strong> {testResults.validation_info.block_type}
                  </div>
                  <div>
                    <strong>Parameters Count:</strong> {testResults.validation_info.parameters_count}
                  </div>
                  <div>
            ...
            {testResults.validation_errors && testResults.validation_errors.length > 0 && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 shadow-sm">
                <h5 className="font-semibold text-yellow-900 mb-2 border-b border-yellow-200 pb-1">Validation Warnings</h5>
...
const BlockDetails: React.FC<BlockDetailsProps> = ({ block, onClose, onSave }) => {
  ...
  // Sub-Tab States
  const [activeDetailsSubTab, setActiveDetailsSubTab] = useState<'info' | 'parameters' | 'io_schema'>('info');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col transform transition-all duration-300">
        <div className="flex justify-between items-center p-5 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800">{block.manifest?.name || block.type} Details</h2>
        ...
        <div className="flex border-b border-gray-200 bg-gray-50 overflow-x-auto">
          ...
          <button
            className={getTabButtonClass('test')}
          ...
          <button
            className={getTabButtonClass('metrics')}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
            ...
            <div className="bg-white p-5 rounded-lg shadow-md h-full">
              ...
              <div className="mt-4 pt-4">
                ...
                {activeDetailsSubTab === 'parameters' && (
                  <div className="space-y-4">
                    <h3 className="text-xl font-bold text-gray-800">Block Parameters Configuration</h3>
                    {isLoading && !schema && (
          ...
          {activeTab === 'test' && (
            <div className="bg-white p-5 rounded-lg shadow-md h-full">
              ...
              <div className="mt-4 pt-4">
                ...
                {activeTestSubTab === 'results' && (
                  <ResultsContent testResults={testResults} />
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === 'metrics' && (
            <div className="bg-white p-5 rounded-lg shadow-md h-full">
              ...
              <div className="mt-4 pt-4">
                {activeAnalyticsSubTab === 'metrics' && (
                  <BlockMetrics blockType={block.type} />
                )}
                ...
                {activeAnalyticsSubTab === 'logs' && (
                  <div className="text-center py-12 text-gray-500 border border-dashed rounded-lg p-8">
                    ...
                    <p className="text-sm">Logs will appear here when the block is executed on the platform.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="border-t border-gray-200 p-5 flex justify-end space-x-3 bg-gray-50 rounded-b-xl">
          ...
          <button
            ...
            title={activeTab !== 'details' || activeDetailsSubTab !== 'parameters' ? "Switch to Parameters tab to save" : "Save the current parameter values"}
          >
            Save Parameters
          </button>
        </div>
