...
const VisualNode: React.FC<NodeProps<VisualNodeData>> = ({ data, selected, id }) => {
  const {
    label,
    blockType,
    category,
  ...
  const getStateIcon = () => {
    switch (executionState) {
      case 'running':
        return <Play className="w-3 h-3 animate-spin" />
      case 'success':
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2">
        ...
        {description && <div className="text-xs text-gray-600 line-clamp-2">{description}</div>}
