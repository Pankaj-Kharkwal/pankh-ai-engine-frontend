...
const ExpressionEditor: React.FC<Props> = ({
  ...
  const insertSuggestion = (suggestion: Suggestion) => {
    ...
    const lastOpen = textBeforeCursor.lastIndexOf('{{')
    const expressionText = textBeforeCursor.substring(lastOpen + 2)
    const currentWordStart = expressionText.lastIndexOf(
      expressionText.split(/[\s.()\[\],]+/).pop() || ''
    )

    const insertPos = lastOpen + 2 + currentWordStart
    const newValue = value.substring(0, insertPos) + suggestion.insertText + textAfterCursor
  ...
  const highlightSyntax = (text: string): string => {
    return text
      ...
      .replace(
        /\b(sum|concat|format|now|random|length|upper|lower|trim)\b/g,
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
        <p>
          Use <code className="px-1 bg-gray-100 dark:bg-gray-700 rounded">{'{{ }}'}</code> for
          expressions. Examples:{' '}
          <code className="px-1 bg-gray-100 dark:bg-gray-700 rounded">{'{{node_1.output}}'}</code>,{' '}
          <code className="px-1 bg-gray-100 dark:bg-gray-700 rounded">{'{{sum(1, 2, 3)}}'}</code>
        </p>
        <p className="mt-1">
          Press <kbd className="px-1 bg-gray-100 dark:bg-gray-700 rounded">Ctrl+Space</kbd> for
          suggestions
        </p>
      </div>
    </div>
