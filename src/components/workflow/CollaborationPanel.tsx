...
const SAMPLE_COMMENTS: Comment[] = [
  {
    ...
    content: 'This AI generator block needs better error handling for API timeouts.',
  ...
  },
  {
    ...
    content: 'The data flow between search and scrape blocks could be optimized.',
...
export default function CollaborationPanel({
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'comments' | 'share'>('comments') // Default to comments for a conversation-focused panel
      ...
      <div className="fixed top-[50%] right-4 transform -translate-y-1/2 z-40">
        <div className="relative">
          ...
          {(onlineUsers.length > 1 || unresolvedComments.length > 0) && (
            <div
              className={`absolute top-0 right-0 w-3 h-3 rounded-full border-2 border-white ${unresolvedComments.length > 0 ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}
        ...
        {activeTab === 'users' && (
          <div className="p-4 space-y-6">
            ...
            <div className="border-t border-gray-200 pt-6">
              <h4 className="text-sm font-bold text-gray-700 mb-3">Recently Active</h4>
              <div className="space-y-3">
                {users
                  .filter(user => !user.isOnline)
                  .slice(0, 3)
                  .map(user => (
                    <div key={user.id} className="flex items-center space-x-3 opacity-80">
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-gray-700 truncate">{user.name}</div>
              ...
              {comments.length === 0 ? (
                ...
                comments.map(comment => (
                  ...
                  >
                    ...
                    {/* Comment Content */}
                    <div className="text-sm text-gray-700 mb-3 ml-8">{comment.content}</div>
                      ...
                      <div className="space-y-3 mb-3 pl-4 border-l-2 border-gray-200 ml-8">
                        {comment.replies.map(reply => (
                          <div key={reply.id} className="text-sm">
                            ...
                            <div className="text-gray-700 ml-6">{reply.content}</div>
                          </div>
                        ))}
                      </div>
                    )}
        ...
        {activeTab === 'share' && (
          <div className="p-4 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Workflow URL</label>
            ...
            <div className="space-y-3 border-t border-gray-200 pt-4">
              <h5 className="text-sm font-bold text-gray-700">Direct Sharing</h5>

              <div className="space-y-2">
                <button
                  onClick={() => copyToClipboard(`Check out this workflow: ${shareUrl}`)}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">Get a direct link to the workflow</div>
                  </div>
                </button>
            ...
            <div className="border-t border-gray-200 pt-4">
              <h5 className="text-sm font-bold text-gray-700 mb-3">Permissions</h5>
