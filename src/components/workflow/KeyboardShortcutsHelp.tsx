...
const categorizeShortcuts = (shortcuts: KeyboardShortcut[]): ShortcutCategory[] => {
  ...
  shortcuts.forEach((shortcut) => {
    ...
    } else if (desc.includes('undo') || desc.includes('redo') || desc.includes('delete') ||
               desc.includes('copy') || desc.includes('paste') || desc.includes('duplicate') ||
               desc.includes('select')) {
      categories['Editing'].push(shortcut);
    } else if (desc.includes('zoom') || desc.includes('search') || desc.includes('panel') ||
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 text-sm text-gray-600">
          <p>
            <strong>Tip:</strong> Press <kbd className="px-2 py-1 bg-white border border-gray-300 rounded text-xs font-mono">Shift+?</kbd> to show/hide this help dialog
          </p>
        </div>
      </div>
    </div>
